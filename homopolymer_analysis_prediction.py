...
def load_gene_sequence_homopolymer(sequence_file_name, name_genes_grna_unique, nuc, polymer_length):
  # Create numpy matrix of size len(name_genes_grna_unique) * 23, to store the sequence as one-hot encoded
  sequence_pam_per_gene_grna = np.zeros((len(name_genes_grna_unique), 23, 4), dtype = bool)
  sequence_genom_context_gene_grna = np.zeros((len(name_genes_grna_unique), 101, 4), dtype=bool)
  # Obtain the grna and PAM sequence corresponding to name_genes_grna_unique
  with open(sequence_file_name) as f:
    for line in f:
      line = line.replace('"', '')
      ...
      if l[1] + '-' + l[0] in name_genes_grna_unique:
        ...
        for i in range(100):
          sequence_genom_context_gene_grna[index_in_name_genes_grna_unique, i, one_hot_index(l[6][i])] = 1
        flag = 0
        start = 50 - int(polymer_length / 2.)
        if polymer_length%2 == 1:
          start = start - np.random.randint(2)
        for i in range(polymer_length):
          if l[6][i+start] != nuc:
            flag = 1
        if flag == 0:
          sequence_genom_context_gene_grna[index_in_name_genes_grna_unique, 100, 0] = 1

  # plot_seq_logo(np.mean(sequence_pam_per_gene_grna, axis=0), "input_spacer")
  # Scikit needs only a 2-d matrix as input, so reshape and return
  return np.reshape(sequence_genom_context_gene_grna, (len(sequence_genom_context_gene_grna), -1))



def load_gene_sequence_homopolymer_spacer(sequence_file_name, name_genes_grna_unique, nuc, polymer_length):
  # Create numpy matrix of size len(name_genes_grna_unique) * 23, to store the sequence as one-hot encoded
  sequence_pam_per_gene_grna = np.zeros((len(name_genes_grna_unique), 24, 4), dtype = bool)
  sequence_genom_context_gene_grna = np.zeros((len(name_genes_grna_unique), 100, 4), dtype=bool)
  # Obtain the grna and PAM sequence corresponding to name_genes_grna_unique
...
p_matrix = np.zeros((4,max_polymer_length-1))

nuc_count = 0
for nuc in ['A', 'C', 'G', 'T']:
  for polymer_length in range(1,max_polymer_length):
    #print "nuc1,nuc2" , nuc1,nuc2
    print "poly length " , polymer_length
    sequence_genom_context_gene_grna = load_gene_sequence_homopolymer(sequence_file_name, name_genes_grna_unique, nuc, polymer_length)
    r2_insertion, r2_deletion, std_r2_insertion, std_r2_deletion, rmse_insertion, rmse_deletion, ins_coef, del_coef = cross_validation_model(sequence_genom_context_gene_grna, prop_insertions_gene_grna, prop_deletions_gene_grna,number_of_repeats)

    insertion_rmse_list[nuc_count, polymer_length-1] =rmse_insertion
    insertion_r2_list[nuc_count, polymer_length-1] = r2_insertion
    insertion_r2_list_std[nuc_count, polymer_length-1] = std_r2_insertion
    tstat, pvalue = ttest_ind_from_stats(r2_insertion_no_polymer, std_r2_insertion_no_polymer, number_of_repeats, r2_insertion, std_r2_insertion, number_of_repeats)
    p_matrix[nuc_count, polymer_length - 1] = -np.log10(pvalue)
...
plt.figure()
for i in range(4):
  plt.errorbar(range(1,max_polymer_length), insertion_r2_list[i,:], yerr=insertion_r2_list_std[i,:], fmt='o')
plt.legend(['A', 'C', 'G', 'T'])
plt.xticks(range(1,max_polymer_length),range(1,max_polymer_length))
plt.xlabel('Homopolymer Length')
plt.ylabel('R^2')
plt.savefig('plots/homopolymer.pdf')
plt.clf()
...
plt.figure()
for i in range(4):
  plt.plot(range(1,max_polymer_length), p_matrix[i,:],'o')
plt.legend(['A', 'C', 'G', 'T'])
plt.xticks(range(1,max_polymer_length),range(1,max_polymer_length))
plt.xlabel('Homopolymer Length')
plt.ylabel('-log(p-value)')
plt.savefig('plots/homopolymer_pvalue.pdf')
plt.clf()

print p_matrix
print insertion_rmse_list
